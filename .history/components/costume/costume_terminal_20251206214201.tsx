...
const AnimatedTerminal = forwardRef((props, ref) => {
  ...
  const currentAudioIndexRef = React.useRef(0)
  ...
  // Initialize audio pool for reliable playback
  useEffect(() => {
    // Prepare a small pool to allow rapid consecutive plays
    const POOL_SIZE = 5
    ...
    // Enable audio on first user interaction and pre-warm buffers
    const enableAudio = () => {
  ...
  // Play typing sound using round-robin pool
  const playTypingSound = () => {
    if (!audioEnabled || audioPoolRef.current.length === 0) return
